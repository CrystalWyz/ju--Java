...
public class FileUtils {
    ...
    public static void tryCreateFile(String filePath) {
        ...
        // 判断文件是否存在
        if (file.isDirectory()) {
            throw new BaseRuntimeException(filePath + " 不是文件目录");
        }
        if (file.exists()) {
        ...
        if (!parentFile.exists()) {
            ...
            if (!mkdirs) {
                throw new BaseRuntimeException("创建目录失败: " + parentFile.getAbsolutePath());
            }
        }
        // 保证了目录存在, 开始创建文件
        try {
            ...
            if (!newFile) {
                throw new BaseRuntimeException("创建文件失败: " + file.getAbsolutePath());
            }
        } catch (Exception e) {
